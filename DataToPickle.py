...
dataset_path, label_path, outfile, training_size, validation_size, testing_size, detect_face, resize = vars(
...
training_size, validation_size, testing_size, resize = training_size / 100, validation_size / 100, testing_size / 100, tuple(
    resize)
...
def load_img_data(files_path, label):
    ...
    for file in files_path:
        if imghdr.what(file) in ['png']:  # Makes sure file is .png image
            ...
            if int(file[
                   -12:-4]) <= threshold:  # Image name are of type 'S005_001_00000002.png' looks at the part '00000002'
...
def serialize_extended_CK(img_path, emotion_label_path):
    ...
    training_data, validation_data, test_data = np.vstack(training_data), np.vstack(validation_data), np.vstack(
        test_data)
    training_label, validation_label, test_label = np.vstack(training_label), np.vstack(validation_label), np.vstack(
        test_label)
